 ...
 * 
 */
package it.unibg.robotics.componentmodels.tca.m2t.handler;

import it.unibg.robotics.componentmodels.m2t.EGLTransformer;
import it.unibg.robotics.componentmodels.tca.m2t.Activator;
...
public class TcaXMLTransform extends AbstractHandler {
	
	public TcaXMLTransform() {	
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (editor != null) {
            ...
            if (input instanceof IFileEditorInput) {
            	...
            	if(sourcefile.getFileExtension().compareTo("tcacomponentmodel") != 0 && sourcefile.getFileExtension().compareTo("tcacomponentmodel_diagram")!=0)
            	{
            		sourcefile = null;
            		MessageDialog
					.openError(
		...
		eglTransformParameter.setTransformName("TCA System to Tuscany XML");
		eglTransformParameter.setEglTransform(fileURL);
		eglTransformParameter.setPluginID(Activator.PLUGIN_ID);
		eglTransformParameter.setSourceMetaModelURI("http://robotics.unibg.it/componentmodels.tca");
		eglTransformParameter.setSourceModelFilePath(sourcefile.getLocation().removeFileExtension().toOSString() + ".tcacomponentmodel");
