 ...
 * 
 */
package it.unibg.robotics.featuremodels.commands.orocosresolution.handlers;
...
import it.unibg.robotics.featuremodels.featureModelPackage;
...
import featureModel.SimpleTransformation;
import featureModel.TransfConnection;
import featureModel.TransfImplementation;
import featureModel.TransfProperty;
import featureModel.rttConnection;
 ...
 */
public class OrocosResolutionHandler extends AbstractHandler {
	...
	private Package targetOrocosModel;

	/**
	 * The constructor.
	 */
	public OrocosResolutionHandler() {
	}
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if(activeEditor instanceof FeatureModelDiagramEditor){
			...
			if(structSelect.getFirstElement() instanceof FeatureEditPart){

				FeatureEditPart featureEditPart = (FeatureEditPart)structSelect.getFirstElement();
				FeatureModel featureModel = featureEditPart.getFeatureModel();

				Package sourceOrocosModel =  featureModel.getRttModel();
				cloneRttPackage(sourceOrocosModel);
				//				targetOrocosModel = EcoreUtil.copy(sourceOrocosModel);


				Instance instance = featureEditPart.getFeatureModel().getSelectedInstance();
...
//				"File Saved");
	...
	private void doTransformation(Instance featureModelInstance){
		...
		for(Feature currentFeature : featureModelInstance.getSelectedFeatures()){
			for(SimpleTransformation currentTransf : currentFeature.getTransormation()){

				if(currentTransf instanceof TransfImplementation){
					TransfImplementation transf = (TransfImplementation)currentTransf;
					TaskContext targetTaskContext = taskContextHashTable.get(getTaskContextHashTableKey(transf.getTaskContext()));
					targetTaskContext.setNamespace(transf.getNewNamespace());
					targetTaskContext.setType(transf.getNewType());

				}else if(currentTransf instanceof TransfConnection){
					TransfConnection transf = (TransfConnection)currentTransf;
					...
					for(rttConnection newRttConnection : transf.getNewRttConnections()){
				...
				}else if(currentTransf instanceof TransfProperty){
					TransfProperty transf = (TransfProperty)currentTransf;

					Property targetProperty = propertyHashTable.get(getPropertyHashTableKey(transf.getProperty()));
					targetProperty.setValue(transf.getNewValue());
